...
- (id) initPrivatelyWithController: (UIViewController *)vc andButtons: (NSArray *) buttons andOffSet: (float) yValue
{
    ...
    _finalBackgroundFrame = CGRectMake((DEVICE_SIZE.width/2)-xSizeOffset-20, (DEVICE_SIZE.height/2)-yValue-20, (size*3)+(20*3)+20, (size*2)+(20*2)+20);
    _backgroundView = [[UIView alloc] init];//[[FXBlurView alloc] init];
    ...
    /* top left */
    _finalItem1Frame = CGRectMake((DEVICE_SIZE.width/2)-xSizeOffset, (DEVICE_SIZE.height/2)-yValue, size, size);
    _initialItem1Frame = CGRectMake(-size*5, -50, size, size);
    if([buttons count] < 1){
        _item1 = [[UIButton alloc] init];
    ...
    _item1.layer.cornerRadius = size/2.0;
    _item1.clipsToBounds = YES;
    [vc.view addSubview:_item1];
    
    /* top center */
    _finalItem2Frame = CGRectMake((DEVICE_SIZE.width/2)-xSizeOffset+size+20, (DEVICE_SIZE.height/2)-yValue, size, size);
    _initialItem2Frame = CGRectMake((DEVICE_SIZE.width/2)-xSizeOffset+60, -size*5, size, size);
    if([buttons count] < 2){
        _item2 = [[UIButton alloc] init];
    ...
    _item2.layer.cornerRadius = size/2.0;
    _item2.clipsToBounds = YES;
    [vc.view addSubview:_item2];
    
    /* top right */
    _finalItem3Frame = CGRectMake((DEVICE_SIZE.width/2)-xSizeOffset+(size*2)+(20*2), (DEVICE_SIZE.height/2)-yValue, size, size);
    _initialItem3Frame = CGRectMake(DEVICE_SIZE.width+size*5, -50, size, size);
    if([buttons count] < 3){
        _item3 = [[UIButton alloc] init];
    ...
    _item3.layer.cornerRadius = size/2.0;
    _item3.clipsToBounds = YES;
    [vc.view addSubview:_item3];
    
    /* bottom left */
    _finalItem4Frame = CGRectMake((DEVICE_SIZE.width/2)-xSizeOffset, (DEVICE_SIZE.height/2)-yValue+size+20, size, size);
    _initialItem4Frame = CGRectMake(-size*5, DEVICE_SIZE.height+size-yValue+20, size, size);
    if([buttons count] < 4){
        _item4 = [[UIButton alloc] init];
    ...
    _item4.layer.cornerRadius = size/2.0;
    _item4.clipsToBounds = YES;
    [vc.view addSubview:_item4];
    
    /* bottom center */
    _finalItem5Frame = CGRectMake((DEVICE_SIZE.width/2)-xSizeOffset+size+20, (DEVICE_SIZE.height/2)-yValue+size+20, size, size);
    _initialItem5Frame = CGRectMake((DEVICE_SIZE.width/2)-xSizeOffset+60, DEVICE_SIZE.height+size*5, size, size);
    if([buttons count] < 5){
        _item5 = [[UIButton alloc] init];
    ...
    _item5.layer.cornerRadius = size/2.0;
    _item5.clipsToBounds = YES;
    [vc.view addSubview:_item5];
    
    
    /* bottom right */
    _finalItem6Frame = CGRectMake((DEVICE_SIZE.width/2)-xSizeOffset+(size*2)+(20*2), (DEVICE_SIZE.height/2)-yValue+size+20, size, size);
    _initialItem6Frame = CGRectMake(DEVICE_SIZE.width+size*5, DEVICE_SIZE.height+size-yValue, size, size);
    if([buttons count] < 6){
        _item6 = [[UIButton alloc] init];
    ...
    _item6.layer.cornerRadius = size/2.0;
...
- (void) setShowsBackgroundFrame:(BOOL) hasFrame
{
    if(hasFrame){
        ...
        if([_backgroundView respondsToSelector:@selector(setTintColor:)]){
            _backgroundView.tintColor = self.transparentTintColour;
        }
    }
    else{
...
- (void) changeTransparentTintColour:(UIColor *)transparentTintColour
{
    ...
    if([_blurredBackgroundView  respondsToSelector:@selector(setTintColor:)]){
